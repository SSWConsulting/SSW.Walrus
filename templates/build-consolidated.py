...
_ADMIN_RE = re.compile(r"free lunch|order form|retreat", re.I)
...
def extract_survey(rows, blocker_cols=()):
    ...
    blocker_cols are the raw headers of the standing blocked question, which is
    reported on its own tab (extract_blockers) and so is kept out of the
    topic questions here.
    """
    if not rows:
        return None
    blocker_cols = set(blocker_cols)
...
# ----------------------------------------------------------------------------
# the standing "was anyone blocking you?" question -> the Blockers tab
# ----------------------------------------------------------------------------
# Every CTF form carries one question that is not about the week's topic: a
# standing escalation valve asking whether the respondent was blocked, plus a
# free-text follow-up naming who and what. It is about people rather than the
# topic, so it gets its own tab instead of being demoted with the free-lunch
# logistics.
#
# Columns are located by header text AND answer vocabulary, never by position.
# Two things break position-based reads: the columns move between years (index
# 20-21 in 2026 forms, 18-19 in 2024 ones), and on some weeks the week's own
# topic question also contains the word "block" (e.g. "How often are you blocked
# waiting on a UI/UX decision?").
...
_SEVERITY_RE = [
    (re.compile(r"more than a day", re.I), "high", "Blocked for more than a day"),
    (re.compile(r"few hours", re.I), "moderate", "Blocked for a few hours"),
    (re.compile(r"about an hour|\ban hour\b", re.I), "low", "Blocked for about an hour"),
]
...
def classify_blocked(ans):
    """yes | no | other-person | na | other-text, or None for an empty answer.
    ...
    if not a:
        return None
    al = a.lower()
    if not _NUMBERED_RE.match(a):
        return "na" if al.startswith("n/a") else "other-text"
    if "someone else" in al:
        return "other-person"
    if "n/a" in al:
        return "na"
    if re.search(r"\byes\b", al):
        return "yes"
    if re.search(r"\bno\b", al):
        return "no"
    return "other-text"


def _blocked_severity(ans):
    """(severity, label), or (None, "") when the answer states no duration.

    The "someone else was blocking me" option carries no duration, so it gets no
    severity rather than a made-up middle one.
    """
    for rx, sev, label in _SEVERITY_RE:
        if rx.search(ans):
            return sev, label
    return None, ""


def _names_in(text, roster, unique_firsts):
    """Roster names that appear in a free-text answer.

    Full name always matches. A bare first name matches only when exactly one
    person in the survey has it — two Toms must never resolve to one of them.
    """
    if not text:
        return []
    found = []
    for full in roster:
        ...
        if not parts:
            continue
        if re.search(r"\b" + re.escape(full) + r"\b", text, re.I):
            found.append(full)
        elif parts[0].lower() in unique_firsts and re.search(
                r"\b" + re.escape(parts[0]) + r"\b", text, re.I):
            found.append(full)
    return found


def extract_blockers(rows, blocked_by=None):
    """Extract who is blocked and what is blocking them, from the raw file.

    Returns None when the survey has no standing blocked question, in which case
    the dashboard omits the Blockers tab entirely.

    blocked_by names the person the question is asked in the voice of ("was I
    ...
    unique_firsts = {f for f, c in first_counts.items() if c == 1}

    owner = (blocked_by or "").strip() or "the person who asked"
    ...
    for r in rows:
        ...
        detail = re.sub(r"\s+", " ", str(r.get(detail_col, "") or "")).strip() if detail_col else ""
        # "N/A" typed into the follow-up box means no detail was given, not a
        # detail that reads "N/A" (same filler set the free-text curation uses).
        if detail.strip().lower().rstrip(".") in _LOW_SIGNAL:
            detail = ""
        if kind == "other-text":
            needs_review.append({"respondent": name or "—", "answer": ans, "detail": detail})
            continue
        if kind not in ("yes", "other-person"):
            continue
        severity, severity_label = _blocked_severity(ans)
        if kind == "yes":
            blocked_by_who = owner
        else:
            named = _names_in(detail, roster, unique_firsts)
            blocked_by_who = ", ".join(named) if named else "Someone else"
        people.append({
            "respondent": name or "—",
            "blockedBy": blocked_by_who,
            "severity": severity,
    ...
    people.sort(key=lambda p: (rank.get(p["severity"], 3), p["respondent"].lower()))
    ...
    return {
        ...
        "blockedRate": (whole(100 * len(people) / answered) or 0) if answered else 0,
        "byOwner": counts["yes"],
        "bySomeoneElse": counts["other-person"],
        "notBlocked": counts["no"],
        "notApplicable": counts["na"],
        "severityCounts": {s: sum(1 for p in people if p["severity"] == s)
...
def build_red_flags(rf):
    ...
    # No blocker signal here: the Blockers tab owns that subject, and it is built
    # from the raw file (extract_blockers) rather than from an agent's count.
    return out
...
def main():
    ...
    ap.add_argument("--blocked-by", default=None,
                    help="who the standing blocked question is asked in the voice of "
                         "(SSW CTF forms: Adam). The file never says who \"me\" is.")
    ...
    # The standing blocked question is pulled out first so extract_survey can
    # hold its columns back — it owns the Blockers tab, not the Responses tab.
    blockers = extract_blockers(grounder.rows, args.blocked_by) if grounder else None
    extracted = extract_survey(
        grounder.rows, blockers["sourceColumns"] if blockers else ()) if grounder else None
    ...
    blocker_names = [b["respondent"] for b in (blockers or {}).get("people", [])]
    all_names = [p["name"] for p in people["respondents"]] + quote_names + blocker_names
    ...
    for b in (blockers or {}).get("people", []):
        b["photoUrl"] = photos.get(b["respondent"])
    ...
    consolidated["blockers"] = blockers
    ...
    consolidated["consolidationNotes"] = {
        ...
        ] + ([f"Extracted {blockers['blockedCount']} blocked people from the standing "
              f"blocked question (Blockers tab)"] if blockers else []),
        "qualityScore": 90,
    ...
    if blockers:
        print(f"  blockers: {blockers['blockedCount']} blocked "
              f"({blockers['byOwner']} by {blockers['blockedBy']}, "
              f"{blockers['bySomeoneElse']} by someone else, "
              f"{blockers['severityCounts']['high']} for more than a day)"
              + (f" — {len(blockers['needsReview'])} free-text answers need a read"
                 if blockers["needsReview"] else ""))
    else:
        print("  blockers: no standing blocked question in this survey — tab omitted")
